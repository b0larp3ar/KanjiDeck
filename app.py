...
def review(level):
    ...
    if a==0:
        cursor.execute("SELECT id from Vocabulary WHERE position=? ORDER BY RANDOM() LIMIT 1", (a,))
        elementid=cursor.fetchone()[0]
        cursor.execute("UPDATE Vocabulary SET position=? WHERE id=?", (1, elementid))
        a+=1

    if card==():
        cursor.execute("SELECT id, original, furigana, english, correct, incorrect, position from Vocabulary WHERE position=? ORDER BY RANDOM()", (a,))
        card=cursor.fetchone()
        
        if card is None:
            cursor.execute("SELECT id, original, furigana, english, correct, incorrect, position FROM Vocabulary WHERE position=0 ORDER BY RANDOM()")
            card=cursor.fetchone()
            cursor.execute("UPDATE Vocabulary SET position=? WHERE id=?", (a, card[0]))
        a+=1

    conn.commit()
    conn.close()
    if card==():
        print("No card selection")
